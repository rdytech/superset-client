...
RSpec.describe Superset::Dashboard::List do
  ...
  describe '#all' do
    it 'returns an array of OpenStruct objects with the correct attributes' do
      ...
      #expect(dashboards.first.schemas).to eq(['acme'])                                   # WIP
      #expect(dashboards.first.allowed_embedded_domains).to eq(['https://test-acme.io/']) # WIP
      #expect(dashboards.first.uuid).to eq('some-uuid-for-36')                            # WIP
    end
  end
