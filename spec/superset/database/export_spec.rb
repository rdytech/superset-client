...
RSpec.describe Superset::Database::Export do
  ...
  describe '#perform' do
    ...
    context 'with remove_dataset_yamls set to true' do
      it 'exports the database zip file and copies it to the destination path and removes dataset yamls' do
        ...
        dataset_files = Dir.glob("#{destination_path_with_db_id}/database_export_20240903T022154/dataset/*.yaml")
    ...
    context 'with remove_dataset_yamls set to false' do
      ...
      it 'exports the database zip file and copies it to the destination path' do
        ...
        dataset_files = Dir.glob("#{destination_path_with_db_id}/*/datasets/examples/*.yaml")
        expect(dataset_files).to match_array([
          "/tmp/some-destination-path/1/database_export_20240903T022154/datasets/examples/users.yaml",
          "/tmp/some-destination-path/1/database_export_20240903T022154/datasets/examples/users_channels-uzooNNtSRO.yaml",
          "/tmp/some-destination-path/1/database_export_20240903T022154/datasets/examples/users_channels.yaml"])
      end
    end
  end
  ...
  describe '#exported_zip_path' do
    ...
    specify do
      expect(subject.exported_zip_path).to eq('/tmp/superset_database_exports/66707162-1231-437a-89db-6a7e2e5929bc/database_1_export_2024-09-03.zip')
    end
  end
end
